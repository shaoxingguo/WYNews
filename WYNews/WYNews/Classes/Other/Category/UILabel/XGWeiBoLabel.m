...
#import "XGWeiBoLabel.h"

@interface XGWeiBoLabel()
...
/* 点击类型 */
@property(nonatomic, assign) XGWeiBoLabelStyle clickStyle;
...
@implementation XGWeiBoLabel


static NSString *const kXGTextRangeKey = @"kXGTextRangeKey";
static NSString *const kXGTextColorKey = @"kXGTextColorKey";
...
- (void)setNormalTextColor:(UIColor *)XGWeiBoLabelNormalColor
{
    _normalTextColor = XGWeiBoLabelNormalColor;
    [self prepareText];
}

- (void)setCustomerMatchArr:(NSArray<NSDictionary *> *)XGWeiBoLabelMatchArr
{
    _customerMatchArr = XGWeiBoLabelMatchArr;
    [self prepareText];
}
...
- (void)setup
{
    ...
    _selectedBackgroundColor = [UIColor colorWithWhite:0.7 alpha:0.2];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.selectedRange.length != 0) {
        ...
        // 2.0.确定颜色
        UIColor *selectedColor = self.isSelected ? self.selectedBackgroundColor : [UIColor clearColor];
...
- (void)prepareTextSystem
{
    ...
    // 1.将布局添加到storeage中
    [self.textStorage addLayoutManager:self.layout];
...
- (void)prepareText
{
    ...
    for (NSValue *value in linkRanges) {
        ...
        [self.textStorage addAttribute:NSForegroundColorAttributeName value:self.linkColor range:range];
    }
    ...
    // 10.匹配用户自定义的字符串
    if (self.customerMatchArr.count > 0) {
        ...
        if (userDefineRangeDicts.count > 0) {
            ...
            for (NSDictionary *dict in userDefineRangeDicts) {
                NSValue *value = dict[kXGTextRangeKey];
                [arrM addObject:value];
                UIColor *color = dict[kXGTextColorKey];
                NSRange range;
                [value getValue:&range];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    switch (self.clickStyle) {
        case XGWeiBoLabelStyleAgreement:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, XGWeiBoLabelStyleAgreement, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XGWeiBoLabelStyleLink:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, XGWeiBoLabelStyleLink, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XGWeiBoLabelStyleTopic:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, XGWeiBoLabelStyleTopic, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XGWeiBoLabelStyleUser:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, XGWeiBoLabelStyleUser, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XGWeiBoLabelStyleUserDefine:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, XGWeiBoLabelStyleUserDefine, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case XGWeiBoLabelStylePhoneNumber:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, XGWeiBoLabelStylePhoneNumber, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
    ...
    // 4.代理
    if ([self.delegate respondsToSelector:@selector(weiBoLabel:didSelectedString:forXGWeiBoLabelStyle:)]) {
        
        [self.delegate weiBoLabel:self didSelectedString:selectedString forXGWeiBoLabelStyle:self.clickStyle];
    }
...
- (NSRange)getSelectRange:(CGPoint)selectPoint
{
    ...
    for (NSValue *value in self.linkRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.clickStyle = XGWeiBoLabelStyleLink;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.clickStyle = XGWeiBoLabelStyleUser;
            return range;
        }
    }
    ...
    for (NSValue *value in self.topicRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.clickStyle = XGWeiBoLabelStyleTopic;
            return range;
        }
    }
    ...
    for (NSValue *value in self.agreementRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.clickStyle = XGWeiBoLabelStyleAgreement;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userDefineRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.clickStyle = XGWeiBoLabelStyleUserDefine;
            return range;
        }
    }
    // 2.6.判断是否是一个电话号码
    for (NSValue *value in self.phoneNumberRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.clickStyle = XGWeiBoLabelStylePhoneNumber;
            return range;
        }
    }
...
- (NSArray<NSDictionary*> *)getUserDefineStringsRange
{
    ...
    if (self.customerMatchArr.count == 0) return nil;
    
    NSMutableArray<NSDictionary*> *arrM = [NSMutableArray array];
    
    NSString *str = [self.textStorage string];
    for (NSDictionary *dict in self.customerMatchArr) {
        ...
        aDictM[kXGTextRangeKey] = value;
        aDictM[kXGTextColorKey] = color;
